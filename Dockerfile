...
RUN apt-get update && apt-get install -y sudo make git devscripts rpm yum-utils pep8 pyflakes uuid-runtime python-flake8 python3-flake8 libtap-formatter-junit-perl koji-1 python-openssl mock build-essential dh-autoreconf pkg-config systemd rubygems sqlite3 qemu-kvm qemu-system-x86 qemu-utils
